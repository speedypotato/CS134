...
//	ofSetWindowShape(1024, 768);
	...
	ot.create(mars.getMesh(0), 10000); //vertices per box
	mars.setRotation(0, 180, 0, 0, 1); //rotate to match octree
}
...
//	cout << ofGetFrameRate() << endl;
	...
	//draw octree
	//ot.drawLeafNodes(ot.root);
	//ot.draw(ot.root, 7, 1, 0);
...
void ofApp::mousePressed(int x, int y, int button) {
	...
	int start = ofGetCurrentTime().getAsMilliseconds();
	TreeNode intersected;
	ot.intersect(ray, ot.root, intersected);
	if (intersected.points.size() > 0) {
		selectedPoint = mars.getMesh(0).getVertex(intersected.points[0]);
		bPointSelected = true;
		cout << ofGetCurrentTime().getAsMilliseconds() - start << "ms to select point." << endl;
	}
}
