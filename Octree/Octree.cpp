...
void Octree::drawLeafNodes(TreeNode & node) {
	if (node.children.size() == 0) { //leaf node
		...
		for (int i = 0; i < node.children.size(); i++) { //keep searching
...
void Octree::create(const ofMesh & geo, int numLevels) {
	...
	root.points = getIndicesInt(mesh);

	subdivide(mesh, root, numLevels, 1);
	cout << "Total Boxes: " << totalNodes << endl;
}

// Recursive subdivide
// @param level NOT USED!!
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
	vector<Box> tempBoxList(8);
	vector<int> indicesList = node.points;
	subDivideBox8(node.box, tempBoxList);

	for (int i = 0; i < tempBoxList.size(); i++) {
		TreeNode tn; //setup treenode for current box
		tn.box = tempBoxList[i];

		vector<int> pointsReturned;
		int numPoints = getMeshPointsInBox(mesh, indicesList, tn.box, pointsReturned);
		tn.points = pointsReturned;

		if (numPoints > 0) { //if points are in subdivided box
			if (numPoints <= numLevels) { //if leaf node
				node.children.push_back(tn);
				totalNodes++;
				cout << "points = " << numPoints - 5 << endl;
			} else { //subdivide and continue
				cout << "subdividing" << endl;
				subdivide(mesh, tn, numLevels, level + 1);
				node.children.push_back(tn);
			}
		}
	}
...
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
	if (node.box.intersect(ray, -1000, 1000)) {
		if (node.children.size() == 0) { //found intersected leaf node
			nodeRtn = node;
			return true;
		} else {
			for (int i = 0; i < node.children.size(); i++) {
...
//Get indices as int vector
vector<int> Octree::getIndicesInt(ofMesh mesh) {
	vector<ofIndexType> in = mesh.getIndices();
	vector<int> returnMe(in.size());
